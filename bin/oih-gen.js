...
async function oihGen() {
  ...
  const outputDir =
    options.output || (await q.ask("Ouput directory", "output"));
  ...
  const defaultConnName = await fse
    .readJson(validatedSpecFile)
    .then(
      (def) =>
        _.kebabCase(def.info.title).replace(/-v-([0-9])+/g, "-v$1") +
        "-connector"
    );
  const connectorName =
    options.name || (await q.ask("Connector name", defaultConnName));
  ...
  console.log(
    "Successfully generated. Connector has been saved in output directory:",
    generatePath
  );
  ...
  // cleanup
  fse
    .remove(downloadedSpecFile)
    .catch((err) =>
      console.error("Could not remove original API specification", err)
    );
  fse
    .remove(validatedSpecFile)
    .catch((err) =>
      console.error("Could not remove validated API specification", err)
    );
}
