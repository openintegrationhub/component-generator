...
const gm = require("gm").subClass({
  appPath: require("graphicsmagick-static").path + "/",
});
...
module.exports = async function generate({
  ...
  let apiTitle =
    api.info.title +
    (suffixServiceNameToTitle ? " (" + api.info["x-serviceName"] + ")" : "");
  ...
  output("lib/triggers/startFlow.js", triggerTemplate, {
    packageName: packageName,
  });
  output("lib/services/process-wrapper.js", wrapperTemplate, {
    packageName: packageName,
  });
  ...
  output(
    "package.json",
    Object.assign(JSON.parse(packageTemplate), {
      name: packageName,
      version: version,
          ...
          Object.keys(resp).forEach((key) => {
            ...
            if (typeof value === "object") {
              ...
              if (
                ...
                !value.data &&
                !value.items &&
                !Object.keys(value).includes(ACTION.PATH.split("/").pop())
              ) {
                console.log("value found 1:", value);
                finalResult(value);
                return false;
              } else if (
                path.split(".").pop() === "attributes" &&
                !value.data &&
                !value.items
              ) {
        ...
        _.forOwn(secReq, (sec, key) => {
          ...
          if (scheme.type === "apiKey") {
            ACTION.SECURITIES.push(
              "securities[" + quote(key) + "] = cfg[" + quote(fieldName) + "]"
            );
          }
          ...
          if (scheme.type === "openIdConnect") {
            ACTION.SECURITIES.push(
              "securities[" + quote(key) + "] = cfg.oauth2"
            );
          }
        });
      }
  ...
  function outputAction(name, params) {
    let content = actionTemplate.replace(
      /\$([a-z_0-9]+)/gi,
      (match, p1) => params[p1]
    );
