...
module.exports = async function generate({inputFile, outputDir, packageName, swaggerUrl, snapshot, suffixServiceNameToTitle=false}) {
    ...
    let actionVariables = ""
    console.log("aciton here",actionVariables)
    ...
    for (let secReq of api.security || []) {
        _.forOwn(secReq, (sec, key) => {
            ...
            if (scheme.type === 'apiKey') {
                ...
                actionVariables = "APIKEY=${{ secrets.apikey }}"
            }
            if (scheme.type === 'http' && scheme.scheme === 'basic') {
                ACTION.SECURITIES.push('securities[' + quote(key) + '] = {username: cfg.username, password: cfg.passphrase}');
                ACTION.TESTSECURITIES.push('securities[' + quote(key) + '] = {username: process.env.USERNAME, password: process.env.PASSPHRASE }');
                actionVariables = "USERNAME=${{ secrets.usernname }} PASSPHRASE=${{ secrets.passphrase }}"
            }
            if (scheme.type === 'http' && scheme.scheme === 'bearer') {
                ...
                actionVariables = "APIKEY=${{ secrets.accesstoken }}"
            }
            if (scheme.type === 'oauth2' && !hasBearer) {
                ACTION.SECURITIES.push('securities[' + quote(key) + '] = { token: { access_token: cfg[' + quote("accessToken") + ']' + '} }' );
                ACTION.TESTSECURITIES.push('securities[' + quote(key) + '] = { token: { access_token: process.env.ACCESSTOKEN  } }' );
                actionVariables = "APIKEY=${{ secrets.accesstoken }}"
            }
            if (scheme.type === 'openIdConnect') {
                ...
                actionVariables = "APIKEY=${{ secrets.oauth2 }}"
            }
        });
    }
    console.log(actionVariables)
    let data = {
        name: 'Tests',
        on: { 
            'push': {
        ...
        'jobs': {
            'build': {
                ...
                'steps': [{'uses': 'actions/checkout@v2'}, {'name': 'Use Node.js ${{ matrix.node-version }}'},{'uses': 'actions/setup-node@v1'},{ 'with':  {'node-version': '${{ matrix.node-version }}'} }, {'run': 'npm ci'},{'run': `${actionVariables} node ./lib/tests/testTrigger.js`},{'run': `${actionVariables} node ./lib/tests/testAction.js`}]
            }
        },
    };
    let yamlStr = yaml.dump(data);
    fs.writeFileSync('./templates/tests.yml', yamlStr, 'utf8');
    const testTemplate = readTemplate('tests.yml');
    output('.github/workflows/tests.yml', testTemplate);
