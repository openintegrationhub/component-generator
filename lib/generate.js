...
module.exports = async function generate({inputFile, outputDir, packageName, swaggerUrl, suffixServiceNameToTitle=false}) {
    ...
    _.forOwn(api.paths, (operations, opPath) => {
        ...
        _.forOwn(operations, (operation, method) => {
            ...
            const recursiveSearch = (respNew,schemaFound) => {

                counter++;
                // console.log("counter", counter);
                if(schemaFound){
                    console.log("RESULT FOUND!!!",respNew)
                    newSchema = respNew;
                    console.log("new Schema:",newSchema);
                    return newSchema; } else {

                // if(typeof respNew === "object"){
                ...
                // console.log("resp 200 new -- :" ,resp200[key]);
                    ...
                    if(typeof value === 'object' ){
                        ...
                        if (key === "properties" && ((!value.data) && (!value.items) && (!Object.keys(value).includes(ACTION.PATH.split("/").pop())) )){
                            console.log("fired1");
                            console.log("new value:", value);
                            return recursiveSearch(value,true);
                        }else if (path.split(".").pop() === "attributes" && (!value.data) && (!value.items)){
                            console.log("fired2");
                        //    console.log("attributes: ", value)
                            recursiveSearch(value,true);

                           
                        } else {
                            console.log("some condition met");
                            return recursiveSearch(value,false); 
                        }                                                                                                                                               
                    }
                    else { 
                        console.log("fired");
            ...
            // }
             ...
             let result = recursiveSearch(resp200,false);
             console.log("final result:",result);
             schemaOut.properties = result;
             console.log("new schema out : ",schemaOut.properties);
             
            // Object.assign(schemaOut.properties,result);
            /// schema out
            // let schemaOut = {
                // type: 'object',
                // properties: {}
            // };
            // let successResp = "200";
            // let respObj = ACTION.responses.successResp; 
            // let objPath;
            // let searchKey = "properties" | "items" | "schema";
            // const recursiveSearch = (respObj, searchKey) => {
                // Object.keys(respObj).forEach(key => {
                //    const value = respObj[key];
                //    objPath = respObj.value;
                //    if(key === searchKey && typeof value === 'object'){
                        // if(key==="properties" && !objPath.data && (properties.value !== ACTION.PATH.split("/").pop())){
                            // schemaOut.properties = ""
                        // }        
            ...
            // _.each(Object.assign({}, pathParams, operation.responses), param => {
                // if(ACTION.responses.successResp.content.application/json.schema.properties.)
// 
            // });
// 
