...
const { toText, transliterateObject, toMD, output, copyTemplate } = require("./functions");
...
async function outputs(packageName, api, swaggerUrl, componentJson, outputDir, apiTitle, snapshot, paginationConfig) {
  ...
  for (const template of templatesToCopy) {
    if (Array.isArray(template)) {
      await copyTemplate(template[0], template[1], outputDir);
    } else {
      await copyTemplate(template, template, outputDir);
    }
  }
  const ACTION = getActionAndSecuritySchemes(api, apiTitle, packageName, snapshot, paginationConfig);

  for (let i = 0; i < contentPathArray.length; i++) {
    let content = contentPathArray[i].template.replace(/\$([a-z_0-9]+)/gi, (match, p1) => ACTION[p1]);
  ...
  await output("lib/utils/helpers.js", templates.helpersTemplate, undefined, outputDir);
  await output("lib/utils/paginator.js", templates.paginatorTemplate, undefined, outputDir);
