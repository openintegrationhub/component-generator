...
function getActionAndSecuritySchemes(api, apiTitle, packageName, snapshot, paginationConfig) {
  const ACTION = {
    API_TITLE: apiTitle,
  ...
  api.components.securitySchemes = api.components.securitySchemes || {};
  ...
  if (secLength === 0) {
    const securities = prompt("What is your security scheme/s ex. ( BasicAuth apiKey OAuth2 bearerAuth )? ");
    const secTypes = securities.split(" ").map((s) => s.trim());
  ...
  for (let secReq of api.security || []) {
    _.forOwn(secReq, (sec, key) => {
      ...
      if (scheme.type === "http" && scheme.scheme === "bearer") {
        ACTION.SECURITIES.push("securities[" + quote(key) + "] = cfg[" + quote("accessToken") + "] || cfg[" + quote("key") + "]");
  ...
  ACTION.PAGINATION_CONFIG = JSON.stringify(paginationConfig);
