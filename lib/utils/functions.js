...
function toMD(text, options = {}) {
  ...
  if (containsHtml(text)) {
    ...
    if (options.minHeaderLevel) {
      let headerBump =
        options.minHeaderLevel - Math.min(...(text.match(/<h[0-9]\b/g) || []).map((h) => Number(h.slice(2))));
...
async function copyTemplate(src, dest = "", outputDir) {
  ...
  await fse.copy(path.join(__dirname, "../../", "templates", src), path.join(outputDir, dest));
}
async function output(filename, text, data, outputDir) {
  if (data) {
    ...
    text = JSON.stringify(text, (key, value) => (key.startsWith("$") ? undefined : value), 4);
  }
