...
const { templates, outputs } = require('./utils');
...
module.exports = async function generate({
  ...
  for (i = 0; i < templates.contentPathArray.length; i++) {
    let params = ACTION;
    let content = templates.contentPathArray[i].template.replace(
      /\$([a-z_0-9]+)/gi,
      (match, p1) => params[p1]
    );
  ...
  await outputs.outputs(packageName, api, swaggerUrl, componentJson, outputDir);
};
