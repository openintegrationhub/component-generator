...
module.exports = function validate(opts) {
  ...
  return parser
    .validate(opts.inputFile, { dereference: { circular: "ignore" } })
    .then((api) => {
      if (api.swagger) {
        ...
        return converter
          ...
          .then((api) =>
            parser.dereference(api, { dereference: { circular: "ignore" } })
          );
      }
      return api;
    })
...
function fixSecurityRequirements(api) {
  if (
    !api.components ||
    _.isEmpty(api.components.securitySchemes) ||
    !_.isEmpty(api.security)
  ) {
    return;
  }
  ...
  api.security = _.keys(api.components.securitySchemes).map((key) => ({
    [key]: [],
  }));
...
function checkIfMultipleOauthFlows(api) {
  ...
  for (let key in schemes) {
    ...
    if (scheme.type === "oauth2" && _.keys(scheme.flows).length > 1) {
      return (
        "Multiple OAuth2 flows defined (" + _.keys(scheme.flows).length + ")."
      );
    }
  }
}
